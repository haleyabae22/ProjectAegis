...
DEFAULT_MODEL = os.getenv("ADK_MODEL", "gemini-3.1-flash-lite-preview")

def populate_user_input(callback_context):
    """
    Callback to ensure 'user_input' is available in the state by pulling
    the most recent user query from the callback context.
    """
    if not callback_context.state.get("user_input"):
        # When running via adk web or runners, the current user message
        # is available in callback_context.user_content
        if callback_context.user_content and callback_context.user_content.parts:
            callback_context.state["user_input"] = callback_context.user_content.parts[0].text
    return None
...
research_agent = Agent(
    ...
    model=DEFAULT_MODEL,
    tools=[google_search],
    before_agent_callback=populate_user_input,
